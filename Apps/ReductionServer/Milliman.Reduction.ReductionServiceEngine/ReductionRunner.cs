    ...
    public class ReductionRunner
    {
        ...
        private void ExecuteActions(string category)
        {
            ...
            foreach (var action in _late_execution[category])
            {
                try
                {
                    _L.DebugFormat("Executing late action for '{0}':\r\n{1}", category, action.Method.ToString());
                    action.Invoke();
                }
                catch (Exception ex)
                {
                    _L.DebugFormat("Fail to run late action '{0}':\r\nERROR: {2}", category, ex.Message);
                }
            }
        ...
        private QMSAPI.DocumentNode GetNode(string qvwName)
        {
            ...
            if (node == null)
            {
                _L.Warn(string.Format("Could not find node '{0}' on Publisher's source documents. The process will be terminated ...", qvwName));
            }
            else
            {
                _L.Info(string.Format("Successfully fetched SourceDocument '{0}' from Qlikview Publisher", qvwName));
            }
            return node;
        }
        ...
        internal enumSummary getSourceDocumentFolder(out QMSAPI.DocumentFolder qmsFolder)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    _L.InfoFormat("Found {0} Publisher. QDS ID used is '{1}'", services.Length, _qds_guid.ToString("N").ToUpper());
                }

                services = _qms_client.GetServices(QMSAPI.ServiceTypes.QlikViewServer);
                if (services == null || services.Length == 0)
                {
                    _L.Warn("QMS did not return any QVS services. Reduction will be continue to run but no statistics will be available for the server or cluster");
                }
                else
                {
                    _qvs_guid = services[0].ID;
                    _L.InfoFormat("Found {0} QlikView Server. QMS ID used is '{1}'", services.Length, _qvs_guid.ToString("N").ToUpper());
                }
        ...
        private enumSummary prepareFileStructureForHierarchy(string sourceFileName, string destinationFolderName, Guid taskId, out string QVWFolderNameGUID, out string QVWFileNameGUID)
        {
            ...
            try
            {
                ...
                _L.Debug(string.Format("Destination folder '{0}' created successfully...", QVWFolderNameGUID));
            ...
            while (is_running)
            {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        _L.Info(string.Format("Task has finished running at {0} (server time). Current status is '{1}' with exit message '{2}'", status.Extended.FinishedTime, status.General.Status.ToString(), Msg));
                        is_running = false;
                        break;
                    }
        ...
        private void AddWrapUpAction(string source, string destination, bool deleteSource)
        {
            ...
            this.AddWrapUpAction(() => {
                try
                {
                    ...
                    if (deleteSource)
                    {
                        _L.InfoFormat("Deleting file '{0}'", source);
                        File.Delete(source);
                    }
                }
                catch (Exception ex)
        ...
        private void AddCleanUpAction(string file, string description = "")
        {
            this.AddCleanUpAction(() => {
                try
                {
                    _L.InfoFormat("Removing file '{0}'", file);
                    if (File.Exists(file))
                        File.Delete(file);
                    else
                        _L.Info("File does not exist...");
                }
                catch (Exception ex)
                {
                    _L.WarnFormat("Unable to clean up file '{0}' - {1}\r\n{2}", file, description, ex.Message);
                }
            });
        }
        ...
        public void CleanUpOrphans()
        {
            ...
            if (orphan_folders.Length > 0)
                _L.InfoFormat("Found {0} orphan folders", orphan_folders.Length);
            else
                _L.Info("No Orphan documents were found for clean up.");
            ...
            foreach (var node in orphan_folders)
            {
                ...
                foreach (var orphan_document_node in ophan_document_folders)
                {
                    try
                    {
                        _L.InfoFormat("Checking tasks for document '{0}'", orphan_document_node.Name);
                        ...
                        foreach (var document_task in orphan_document_tasks)
                        {
                            try
                            {
                                _L.InfoFormat("Deleting task '{0}' of type '{1}'", document_task.Name, document_task.Type.ToString());
                                _qms_client.DeleteTask(document_task.ID, document_task.Type);
                            }
                            catch (Exception task_exception)
                            {
                                error_count++;
                                _L.ErrorFormat("Error while trying to delete task '{0}':\r\n\t\t{1}", document_task.Name, task_exception.Message);
                            }
                        }
                    }
                    ...
                    {
                        error_count++;
                        _L.ErrorFormat("Error while trying to get documents for folder '{0}':\r\n\t\t{1}", orphan_document_node.Name, orphan_document_exception.Message);
                    }
                }
            }
            if (error_count > 0)
                _L.InfoFormat("Sanitation complete with {0} error{1}.", error_count, error_count > 1 ? "s" : "");
            else
                _L.Info("Sanitation was completed with no errors.");
        }
