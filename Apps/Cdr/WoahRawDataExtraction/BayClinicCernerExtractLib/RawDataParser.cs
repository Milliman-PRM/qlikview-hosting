...
namespace BayClinicCernerExtractLib
{
    ...
    public class RawDataParser
    {
        ...
        public RawDataParser(string IniFile, string SectionName)
        {
            ...
            Trace.WriteLine("Bay Clinic raw data extraction process launched " + DateTime.Now.ToString());
        ...
        public void MigrateFolderToMongo(string zipFolder, bool InsertToMongo = false, String ArchiveFolder = null)
        {
            ...
            MongoDbConnection MongoCxn = InitializeMongoConnection(IniFileName, IniSectionName);
            if (!MongoCxn.IsInitialized)
            {
                Trace.WriteLine("MongoDB connection failed validation");
                throw new Exception("MongoDB connection failed validation");
            }
            ...
            foreach (string Zip in Directory.GetFiles(zipFolder, @"*.zip").OrderBy(name => Directory.GetLastWriteTime(name)))
            {
                ...
                // add doc to MongoDB to record that this zip is being imported
                if (InsertToMongo)
                {
                    InsertDict.Add("zipfile", Path.GetFileName(Zip));
                    InsertDict.Add("import_date_time", DateTime.Now.ToString());
            ...
            Trace.WriteLine("Total zip files: " + ZipFileCount.ToString());
        ...
        public void MigrateZipFileToMongo(string ZipFileName, bool InsertToMongo = false, String ArchiveFolder = null, MongoDbConnection MongoCxn = null)
        {
            ...
            {
                MongoCxn = InitializeMongoConnection(IniFileName, IniSectionName);
            }
        ...
        /// <summary>
        /// Selects the source of configuration parameters and initiates the establishment of a connection to MongoDB
        /// </summary>
        /// <param name="NewBayClinicEmrMongoCredentialConfigFile">Maybe null if supporting environment variables are expected</param>
        /// <param name="NewBayClinicEmrMongoCredentialSection">Maybe null if supporting environment variables are expected</param>
        /// <returns>The resulting connection</returns>
        private MongoDbConnection InitializeMongoConnection(String NewBayClinicEmrMongoCredentialConfigFile, String NewBayClinicEmrMongoCredentialSection)
        {
            MongoDbConnection NewCxn = new MongoDbConnection();
            ...
            if (!Environment.ExpandEnvironmentVariables("<%ephi_username%><%ephi_password%><%Mongo.UserDomain%><%Mongo.Host%>").Contains("<%"))
            {
                ...
                if (NewCxn.InitializeWithEnvironment(FeedIdentity))
                {
            ...
            else
            {
                ...
                if (NewCxn.InitializeWithIni(NewBayClinicEmrMongoCredentialConfigFile, NewBayClinicEmrMongoCredentialSection))
                {
            ...
            if (NewCxn.GetCollectionNames().Count > 1)
            {
