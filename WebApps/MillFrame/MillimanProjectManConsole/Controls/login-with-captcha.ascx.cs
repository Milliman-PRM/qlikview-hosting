...
public partial class controls_login_with_captcha : System.Web.UI.UserControl
{
    ...
    {
        //uncomment to test registering
        //MillimanProjectManConsole.ProductionPublisher PP = MillimanProjectManConsole.Global.Publisher;
        //PP.TestRegister();

        //VWN login helper
        //Login Loginx = GetLogin1();
        //if ( Loginx != null )
        //   Loginx.UserName = "van.nanney@milliman.com";
       

        //log user out to prevent silly blank login screen
        if (!Page.IsPostBack)
        {
    ...
    /// <summary>
    /// Sync local cache to remote server authentication cache
    /// then check local cache
    /// </summary>
    /// <param name="Accountname"></param>
    /// <param name="Password"></param>
    /// <returns></returns>
    private void SyncToRemoteServer(string Accountname, string Password)
    {
        try
        {
            MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, "starting sync");
            MillimanCommon.AutoCrypt AC = new MillimanCommon.AutoCrypt();
            string EncryptedPassword = AC.AutoEncrypt(Password);
            bool IsValid = (MillimanProjectManConsole.Global.GetInstance().ValidUser(Accountname, EncryptedPassword));
            MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, "valid user? " + IsValid.ToString());
            if (IsValid)
            {
                MembershipUser MU = Membership.GetUser(Accountname);

                if (MU == null)
                {
                    MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, "creating user");
                    MU = Membership.CreateUser(Accountname, Password, Accountname);
                }
                if ( Roles.IsUserInRole( Accountname, "Administrator" ) == false )
                    Roles.AddUserToRole(MU.UserName, "Administrator");//always make cache role admin
                if (MU.ChangePassword(MU.GetPassword(), Password) == true)
                    MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, Accountname + " synced");
            }
            else
            {
                //did not validate, so delete from cache
                //VWN do not do during testing
                //Membership.DeleteUser(Accountname);
                //VWN correct group length issue in WSAT
                //VWN rename group
            }
        }
        catch (Exception ex)
        {
            MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Error,"Sync Error", ex);
        }
    ...
    protected void Login1_LoggedIn(object sender, EventArgs e)
    {
        ...
        Response.Redirect("~/Admin/publisher.aspx");
