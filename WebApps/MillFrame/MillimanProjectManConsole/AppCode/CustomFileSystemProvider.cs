...
{
    //private string GROUP_ID = "clientcode";
	...
	private string pathToConfigFile = "~/AppCode/MappingFile.mapping";
	...
	private FileItem[] GetFiles(string virtualPath)
	{
		...
		try
		{
            ...
            MillimanCommon.ProjectSettings PS = null;
			foreach (string filePath in files)
			{
                ...
                //add data for custom data here
                fileItem.Attributes.Add("LocalSaved", fileInfo.LastWriteTime.ToString());
                PS = MillimanCommon.ProjectSettings.Load( Path.Combine( physicalPath , fileItem.Name ));
                if ( MillimanProjectManConsole.Global.ServerLocalProjectsSame( PS ) == false )
                    fileItem.Attributes.Add("LocalServer", "<b><font color='red' size='5px'><center>&#x2260;</center></font></b>");
                else
                    fileItem.Attributes.Add("LocalServer", "<b><font color='green' size='5px'><center>=</center></font></b>");

                fileItem.Attributes.Add("ServerUpdated", ((PS == null) || (string.IsNullOrEmpty(PS.UploadedToProductionDate))) ? "" : PS.UploadedToProductionDate.ToString());
                fileItem.Attributes.Add("ServerUpdatedBy", ((PS == null) || (string.IsNullOrEmpty(PS.UploadedToProduction))) ? "" : PS.UploadedToProduction);

                //fileItem.Attributes.Add("Resource", ((PS == null) || (string.IsNullOrEmpty(PS.QVResources))) ? "" : Path.GetFileName(PS.QVResources));
                //fileItem.Attributes.Add("Thumbnail", ((PS == null) || (string.IsNullOrEmpty(PS.QVThumbnail))) ? "" : Path.GetFileName(PS.QVThumbnail));
                string GroupName = string.Empty;
                //check signage
                if (PS != null)
                {
                    string QVW = System.IO.Path.Combine(PS.LoadedFromPath, PS.QVName +".qvw");
                    if (System.IO.File.Exists(QVW))
                    {
                        MillimanCommon.XMLFileSignature XMLFS = new MillimanCommon.XMLFileSignature(QVW);
                        foreach (KeyValuePair<string, string> Item in XMLFS.SignatureDictionary)
                        {
                            if (Item.Key.StartsWith("@") == true)
                            {
                                if ((string.IsNullOrEmpty(GroupName) == false) && (string.IsNullOrEmpty(Item.Value) == false) )
                                    GroupName += "_";
                                GroupName += Item.Value;
                            }
                        }
                        if (string.IsNullOrEmpty(GroupName) == true)
                            GroupName = "<center>*</center>";
                        else
                            GroupName = "<center>" + GroupName + "</center>";
                    }
                }
                fileItem.Attributes.Add("Group", GroupName);
                fileItem.Attributes.Add("Description", ((PS == null) || (string.IsNullOrEmpty(PS.QVDescription))) ? "" : Path.GetFileName(PS.QVDescription));
                fileItem.Attributes.Add("CovisintField", ((PS == null) || (string.IsNullOrEmpty(PS.CovisintFieldName))) ? "" : Path.GetFileName(PS.CovisintFieldName));
                fileItem.Attributes.Add("MillimanField", ((PS == null) || (string.IsNullOrEmpty(PS.MillimanControlName))) ? "" : Path.GetFileName(PS.MillimanControlName));
                fileItem.Attributes.Add("FriendlyDBName", ((PS == null) || (string.IsNullOrEmpty(PS.FriendlyDBName))) ? "" : Path.GetFileName(PS.FriendlyDBName));
                ...
                //AddColumnToGrid("Server Updated by", "ServerUpdatedBy", 100);
                ////AddColumnToGrid("Resource", "Resource", 80);
                ////AddColumnToGrid("Thumbnail", "Thumbnail", 120);
                //AddColumnToGrid("Description", "Description", 350);
                ...
                //VWN yes the below works to add an image to the grid
                //fileItem.Attributes.Add("FriendlyDBName", "<img src='http://127.0.0.1/MPMC/images/ajax-loader.gif' class='thumbImages'/>");
    ...
    private bool IsHiddenDirectory(string QualifiedPath)
    {
        string HiddenDirectories = System.Configuration.ConfigurationManager.AppSettings["hidden_dirs"].ToLower();
        //VWN
        //if a path ends in "_data" we check to see if there is a project there by the same name, if there is
        //then the "_data" folder should not be displayed but treated as a hidden folder
        if (QualifiedPath.ToLower().EndsWith("_data") == true)
        {
            string HCIPRJ = QualifiedPath.ToLower().Replace("_data", ".hciprj");
            if (System.IO.File.Exists(HCIPRJ) == true)
                return true; //this is  a system hidden directory
        }

        if (string.IsNullOrEmpty(HiddenDirectories) == false)
        {
            string[] HideDirs = HiddenDirectories.Split(new char[] { '~' });
            foreach (string HD in HideDirs)
            {
                if (string.Compare(PathHelper.GetDirectoryName(QualifiedPath), HD, true) == 0)
                    return true;
            }
        }

        //else if (string.Compare(PathHelper.GetDirectoryName(QualifiedPath), "history", true) == 0)
        //{
        //    return true;
        //}
        //else if (string.Compare(PathHelper.GetDirectoryName(QualifiedPath), "ReducedUserQVWs", true) == 0)
        //{
        //    return true;
        //}
        return false;
	...
	private DirectoryItem[] GetDirectories(string virtualPath)
	{
		...
		try
		{
			...
			{
                //if (string.Compare(PathHelper.GetDirectoryName(dirPath), "history", true) != 0)
                if (IsHiddenDirectory( dirPath) == false )
                {
