using System.Security.Cryptography.X509Certificates;
using System.Web.Configuration;
using System.Xml;
using System.Web;
using System.Web.Security;
using ComponentPro.Saml;
using ComponentPro.Saml2;
using ComponentPro.Saml2.Binding;
using System.Collections;
using System.Collections.Generic;

namespace MillimanDev
{
    public partial class AssertionService : System.Web.UI.Page
    {
        /// <summary>
        /// Receives the SAML response from the identity provider.
        /// </summary>
        /// <param name="samlResponse"></param>
        /// <param name="relayState"></param>
        private void ReceiveResponse(out ComponentPro.Saml2.Response samlResponse, out string relayState)
        {
            // Determine the identity provider to service provider binding type.
            // We use a query string parameter rather than having separate endpoints per binding.
            string bindingType = Request.QueryString[Util.BindingVarName] == null ? "SamlBindingUri.HttpPost" : Request.QueryString[Util.BindingVarName];
           
            switch (bindingType)
            {
                case "SamlBindingUri.HttpPost":
                    MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, "HTTP Post Assertion");
                    MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Info, Request.Params.ToString());
                    samlResponse = ComponentPro.Saml2.Response.Create(Request);
                    relayState = samlResponse.RelayState;
                    break;

                case "SamlBindingUri.HttpArtifact":
                    Saml2ArtifactType0004 httpArtifact = Saml2ArtifactType0004.CreateFromHttpArtifactHttpForm(Request);

                    // Create an artifact resolve request.
                    ArtifactResolve artifactResolve = new ArtifactResolve();
                    artifactResolve.Issuer = new Issuer(Util.GetAbsoluteUrl(this, "~/"));
                    artifactResolve.Artifact = new Artifact(httpArtifact.ToString());

                    // Send the artifact resolve request and receive the artifact response.
                    string spArtifactResponderUrl = SSOConfiguration.IdPArtifactIdProviderUrl;

                    ArtifactResponse artifactResponse = ArtifactResponse.SendSamlMessageReceiveAftifactResponse(spArtifactResponderUrl, artifactResolve);

                    // Extract the authentication request from the artifact response.
                    samlResponse = new Response(artifactResponse.Message);
                    relayState = httpArtifact.RelayState;
                    break;

                default:
                    Trace.Write("ServiceProvider", "Invalid identity provider to service provider binding");
                    samlResponse = null;
                    relayState = null;
                    return;
            }

            // Verify the response's signature.
            //original code modified to deal with multiple certificates
            //X509Certificate2 x509Certificate = (X509Certificate2) Application[Global.IdPCertKey];

            //if (!samlResponse.Validate(x509Certificate))
            //{
            //    throw new System.ApplicationException("The SAML response signature failed to verify.");
            //}


            //check all the certificates instead of just one
            // if needed we could look at the cert to figure out which covisint system it came from
            if ( IsTrusted( ref samlResponse ) == false )
            {
                throw new System.ApplicationException("The SAML response signature failed to verify.");
            }
        }

        /// <summary>
        /// Loop over all certs we loaded with the application,  if one matches we accept the connection
        /// </summary>
        /// <param name="samlResponse"></param>
        /// <returns>True if the connection is valid</returns>
        private bool IsTrusted(ref ComponentPro.Saml2.Response samlResponse)
        {
            try
            {
                //added so we can test with NextGen
                if (System.Configuration.ConfigurationManager.AppSettings["IgnoreTrustCertificates"] != null)
                {
                    try  //don't let this crash us, if ingoretrustcertificats is invalid, go ahead and check certs are normal
                    {
                        if (System.Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings["IgnoreTrustCertificates"]) == true)
                            return true;  //we will allow anyone in
                    }
                    catch (System.Exception)
                    {
                    }
                }
                string KeyRoot = @"CovisintCert";
                int Index = 1;
                string CurrentKey = KeyRoot + @"_" + Index.ToString();
                while (Application[CurrentKey] != null)
                {
                    X509Certificate2 x509Certificate = (X509Certificate2)Application[CurrentKey];
                    if (samlResponse.Validate(x509Certificate))
                        return true;

                    Index++;
                    CurrentKey = KeyRoot + @"_" + Index.ToString();
                }
            }
            catch (System.Exception ex)
            {
                MillimanCommon.Report.Log(MillimanCommon.Report.ReportType.Error, "Invalid certificate '.crt' found in store [ SSOConfigure directory]" + ex.ToString());
            }
            return false;
        }

        /// <summary>
        /// Processes a successful SAML response.
        /// </summary>
        private void ProcessSuccessResponse(ComponentPro.Saml2.Response samlResponse, string relayState)
        {
            // Extract the asserted identity from the SAML response.
            Assertion samlAssertion = (Assertion)samlResponse.Assertions[0];
            
            // Get the subject name identifier.
            string userName = samlAssertion.Subject.NameId.NameIdentifier;
            string Context = "";
            string ApplicationID = "";
            foreach (AttributeStatement AS in samlAssertion.AttributeStatements)
            {
                foreach (Attribute A in AS.UnencryptedAttributes)
                {
                    //patient id is provided via the URL, not in an assertion
                    if (string.Compare("patientid", A.Name, true) == 0)
                    {
                        Context = A.Values[0].ToString();
                        if (string.IsNullOrEmpty(Context) == false)
                            Context = Dashboard.MapToInternalID(Context);
                    }

                    else if (string.Compare("covisintapplicationid", A.Name, true) == 0)
                    {
                        //HACK:set the app id to the report name - we will use this below if provided
                        ApplicationID = A.Values[0].ToString(); //first id is staging - papc10143671  second is production - papc11408229
                        if ((string.Compare(ApplicationID, "papc10143671", true) == 0) || (relayState.ToLower().Contains("papc10143671") == true) ||
                            (string.Compare(ApplicationID, "papc11408229", true) == 0) || ( relayState.ToLower().Contains("papc11408229") == true ) )
                            ApplicationID = "POPULATION";
                        else if ((string.Compare(ApplicationID, "papc10143675", true) == 0) || (relayState.ToLower().Contains("papc10143675") == true) || 
                                 (string.Compare(ApplicationID, "papc11408223", true) == 0) || (relayState.ToLower().Contains("papc11408223") == true) )
                            ApplicationID = "COSTMODEL";
                    }
                }
            }

            //patient id is always supplied via url
            //PatientID = string.IsNullOrEmpty(PatientID) ? "[Not Specified]" : PatientID;
            //Session["patientid"] = @"Covisint PatientID:" + PatientID;
            // Get the originally requested resource URL from the relay state.
            string resourceUrl = relayState != null ? (string)SamlSettings.CacheProvider.Remove(relayState) : string.Empty;

            if ( string.IsNullOrEmpty(resourceUrl))
            {
                Trace.Write("ServiceProvider", "Nothing in cache");

                resourceUrl = "Default.aspx";
            }

            if ((Membership.ValidateUser(userName, MillimanCommon.Predefined.DefaultCovisintPassword) == true) || (Membership.ValidateUser(userName, MillimanCommon.Predefined.ExternalUserSSOPassword)))
            // if (FormsAuthentication.Authenticate(userName, Global.DefaultCovisintPassword) == true)
            {
                // Create a login context for the asserted identity.
                FormsAuthentication.SetAuthCookie(userName, false);

                //make sure the origin on the URL is set to Covisint
                Dictionary<string,string> MyItemsRaw = Util.ParseQueryString(Request.RawUrl);
                Dictionary<string, string> MyItems = new Dictionary<string, string>();
                foreach (KeyValuePair<string, string> S in MyItemsRaw)  //bad, make case insensitive, dictionary should do this
                {
                        MyItems.Add(S.Key.ToLower(), S.Value);
                }
                //URL may provide the dashboard id,  or it could be we are being pinged from a specific Cov app entry point
                string DashboardID = MyItems.ContainsKey("dashboardid") ? MyItems["dashboardid"] : ApplicationID;
                //string Context = PatientID;
                //if ( string.IsNullOrEmpty(Context ) )
                //    Context = MyItems.ContainsKey("f_patientid") ? MyItems["f_patientid"] : "";
                //if ( string.IsNullOrEmpty( Context ) )
                //    Context = MyItems.ContainsKey("enterpriseid") ? MyItems["enterpriseid"] : "";
                Dashboard.SystemOrigin Origin = Dashboard.SystemOrigin.ExternalSSO;  //always covisint

                //we look to see if there was an enterprise id on the assertion URL
                if (string.IsNullOrEmpty(Context) == false)
                    Session["patientid"] = Context;
                else if (Session["patientid"] != null)
                    Session.Remove("patientid");

                if ( string.IsNullOrEmpty(DashboardID) == false )
                    resourceUrl = Dashboard.URLBuilder(DashboardID, Context, Origin);
            }
            else
            {
               resourceUrl = "FederatedLoginIssue.html";
            }

            // Redirect to the originally requested resource URL.
            Response.Redirect(resourceUrl);
        }

        /// <summary>
        /// Processes an error SAML response. 
        /// </summary>
        private void ProcessErrorResponse(ComponentPro.Saml2.Response samlResponse)
        {
            string errorMessage = null;

            if ((samlResponse.Status.StatusMessage != null))
            {
                errorMessage = samlResponse.Status.StatusMessage.Message;
            }

            string redirectUrl = string.Format("~/LoginChoice.aspx?{0}={1}", Util.ErrorVarName, HttpUtility.UrlEncode(errorMessage));

            Response.Redirect(redirectUrl, false);
        }

        protected override void OnLoad(System.EventArgs e)
        {
            base.OnLoad(e);

            try
            {
                // Process the SAML response returned by the identity provider in response
                // to the authentication request sent by the service provider.

                // Receive the SAML response.
                ComponentPro.Saml2.Response samlResponse;
                string relayState;

                ReceiveResponse(out samlResponse, out relayState);

                if (samlResponse == null) return;


                // Check whether the SAML response indicates success or an error and process accordingly.
                if (samlResponse.IsSuccess())
                {
                    ProcessSuccessResponse(samlResponse, relayState);
                }
                else
                {
                    ProcessErrorResponse(samlResponse);
                }
            }

            catch (System.Exception exception)
            {
                Trace.Write("ServiceProvider", "Error in assertion consumer service", exception);
            }
        }
    }
}