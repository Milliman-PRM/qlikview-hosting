...
public partial class admin_controls_email_broadcast : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string[] Templates = System.IO.Directory.GetFiles(EmailTemplatesDir, "*.htm");

            EmailTemplates.Items.Add(new ListItem("Empty", ""));

            foreach (string Template in Templates)
            {
                string DisplayName = System.IO.Path.GetFileNameWithoutExtension(Template).Replace("-", " ");
                EmailTemplates.Items.Add(new ListItem(DisplayName, Template));
            }

            MacroList.Items.Add( new ListItem("<%username%>","<%username%>"));
            MacroList.Items.Add( new ListItem("<%date%>","<%date%>"));
            MacroList.Items.Add(new ListItem("<%datetime%>", "<%datetime%>"));
            MacroList.Items.Add( new ListItem("<%securelink%>","<%securelink%>"));
            MacroList.Items.Add(new ListItem("<%millimanlogo%>", "<%millimanlogo%>"));
            MacroList.Items.Add(new ListItem("<%hcintellogo%>", "<%hcintellogo%>"));
            MacroList.Items.Add(new ListItem("<%siteurl%>", "<%siteurl%>"));
    ...
    {
        if (string.IsNullOrEmpty(EmailTemplates.SelectedValue) == false)
        {  //no need to map, contains absolute server path
            WYSIWYGEditor_EmailBody.Value = System.IO.File.ReadAllText(EmailTemplates.SelectedValue);
        }
        else  //no template, clear it
        {
            WYSIWYGEditor_EmailBody.Value = "";
        }
    }
    protected void MacroList_SelectedIndexChanged(object sender, EventArgs e)
    {
       //may have to do this client side
    }
}