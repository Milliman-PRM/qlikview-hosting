...
namespace ReportingCommon
{
    ...
    public class IisLogFileEntry
    {
        ...
        /// <param name="LogFileName">The end of the defined date range</param>
        /// <returns>List<IisLogFileEntry></returns>
        public static List<IisLogFileEntry> ParseOneIisLogFile(string logFillFullName, string qvDocRoot)
        {
            string[] supportedIisVersion = { "8.5" };
            List<IisLogFileEntry> listLogFile = new List<IisLogFileEntry>();

            List<string> fileLines = File.ReadAllLines(logFillFullName).ToList();
            ...
            if (supportedIisVersion.Contains(thisIisVersion))
            {
                ...
                {
                    List<UserEventBase> listUserEventBase = new List<UserEventBase>();
                    foreach (string row in fileLines)
                    {
                        IisLogFileEntry iIsLogEntry = new IisLogFileEntry(row);
                        if (iIsLogEntry != null)
                        {
                            string group = IisAccessEvent.DecodeUriQuery(iIsLogEntry.UriQuery, qvDocRoot);
                            iIsLogEntry.UserName = (!string.IsNullOrEmpty(iIsLogEntry.UserName)) ? iIsLogEntry.UserName.Trim() : "UnKnownUser";
                            iIsLogEntry.UriQuery = (!string.IsNullOrEmpty(group)) ? group.Trim() : string.Empty;
                        }
                        listLogFile.Add(iIsLogEntry);
                    }
                }                
            }
